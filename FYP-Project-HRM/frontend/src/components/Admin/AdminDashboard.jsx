...
const AdminDashboard = () => {
  ...
  // Build axios instance with token — called once per load
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Check if any call returned 401 — token is invalid/expired
        ...
        if (results[0].status === 'fulfilled' && results[0].value.data.success) {
          ...
          // Stats is critical — surface the error
  ...
  const TeamMember = ({ member, index }) => (
    ...
    >
      <div className="relative">
        ...
        <div className={`w-2 h-2 rounded-full border-2 ${darkMode ? 'border-gray-900' : 'border-white'} absolute -top-0.5 -right-0.5 ${
      ...
      <div className="text-right">
        ...
        <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Progress</p>
      </div>
    </div>
  );
    ...
    <div className={`min-h-screen py-8 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Total Employees" value={stats.totalEmployees} icon="👥" color="bg-gradient-to-br from-blue-500 to-cyan-500" delay={0} />
          <StatCard title="Active Departments" value={stats.activeDepartments} icon="🏢" color="bg-gradient-to-br from-green-500 to-emerald-500" delay={100} />
          <StatCard title="System Health" value={stats.systemHealth} icon="💚" color="bg-gradient-to-br from-purple-500 to-pink-500" delay={200} suffix="%" />
          <StatCard title="Employee Satisfaction" value={stats.employeeSatisfaction} icon="⭐" color="bg-gradient-to-br from-amber-500 to-orange-500" delay={300} suffix="%" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Quick Actions */}
            <div className={`rounded-2xl shadow-2xl border p-6 animate-fade-in-up ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
              <h2 className={`text-xl font-semibold mb-6 ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Quick Actions</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {quickActions.length > 0 ? quickActions.map((action) => (
                  ...
                  <p className={`col-span-2 text-center py-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            {recentActivity.length > 0 && (
              <div className={`rounded-2xl shadow-2xl border p-6 animate-fade-in-up ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
                <h2 className={`text-xl font-semibold mb-6 ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Recent Activity</h2>
          ...
          <div className="space-y-6">
            ...
            {notifications.length > 0 && (
              <div className={`rounded-2xl shadow-2xl border p-6 animate-fade-in-up ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`text-lg font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Notifications</h3>
                  {unreadCount > 0 && (
            ...
            {performanceData.length > 0 && (
              <div className={`rounded-2xl shadow-2xl border p-6 animate-fade-in-up ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
                <h3 className={`text-lg font-semibold mb-4 ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>System Performance</h3>
                <div className="space-y-4">
            ...
            {teamMembers.length > 0 && (
              <div className={`rounded-2xl shadow-2xl border p-6 animate-fade-in-up ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
                <h3 className={`text-lg font-semibold mb-4 ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Team Overview</h3>
