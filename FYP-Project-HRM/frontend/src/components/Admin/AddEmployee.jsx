...
const AddEmployee = () => {
  ...
  // ─── FIX: explicit map so camelCase field names never produce wrong show-field names ───
  const showFieldMap = {
    ...
    systemRole:      'showSystemRoleCustom',   // ← was being built incorrectly before
  };
  ...
  // ─── FIX: uses showFieldMap so systemRole → showSystemRoleCustom correctly ───
  ...
  const validateForm = () => {
    ...
    if (formData.idCardIssueDate && formData.idCardExpiryDate) {
      if (new Date(formData.idCardIssueDate) > new Date(formData.idCardExpiryDate)) {
        newErrors.idCardIssueDate = 'Issue date cannot be after expiry date';
      }
    }
    
    if (formData.idCardExpiryDate) {
      if (new Date(formData.idCardExpiryDate) < new Date()) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ─── FIX: userAccount.role now uses the selected systemRole instead of hardcoded 'employee' ───
      ...
      const employeeData = {
        userAccount: {
          ...
          role:     resolvedSystemRole   // ← was always 'employee' before this fix
        },
        ...
        employeeProfile: {
          ...
          role:            resolvedSystemRole,          // ← same resolved value, consistent
          ...
          <form onSubmit={handleSubmit} className="divide-y divide-gray-200">
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {/* System Role — this is the field that was broken */}
                <div>
