...
// ─── AI Attendance Modal (Webcam only — no upload tab) ────────────────────────
...
const AIAttendanceModal = ({ type, isOpen, onClose, onSuccess }) => {
  ...
  const getGPS = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        else if (err.code === 3) errorMsg = 'Location timed out';
        else                     errorMsg = 'Unknown GPS error';
        setGpsStatus('error');
  ...
  const handleSubmit = async () => {
    ...
    if (gpsStatus !== 'ok' || !gpsData) {
      alert('Please allow location access first and wait for GPS to load');
      return;
    }
    setSubmitting(true);
    setResult(null);
    try {
      ...
      let errorMessage = body.message || 'Attendance failed. Please try again.';
      if (body.stage === 'gps') {
        errorMessage = `📍 GPS Error: ${body.message} (Distance: ${body.distance}m, Max: ${body.maxDistance || 100}m)`;
      } else if (body.stage === 'face') {
        errorMessage = `😀 Face Recognition Error: ${body.message}`;
      }
      setResult({
        success: false,
  ...
  const retakePhoto = () => { setCapturedImage(null); setCamStatus('idle'); setResult(null); };

  // Reset & start GPS + camera whenever modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  // Auto-start camera when modal is open and camera is idle
  useEffect(() => {
  ...
  const canSubmit = gpsOk && !!capturedImage && !submitting;
  const gpsLabel  = gpsStatus === 'loading' ? 'Getting location…'
    : gpsStatus === 'ok'    ? `${gpsData?.lat.toFixed(4)}, ${gpsData?.lng.toFixed(4)} (±${gpsData?.accuracy}m)`
    : gpsStatus === 'error' ? `Error: ${gpsError}`
    : 'Not captured';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className={`px-6 py-4 flex items-center justify-between ${isCheckIn ? 'bg-emerald-600' : 'bg-rose-600'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-xs">Live webcam · Face recognition + GPS required</p>
            </div>
          </div>
        ...
        <div className="px-6 pt-4 pb-2 flex items-center gap-3 border-b border-gray-100">
          {[['1', 'Location', gpsOk], ['2', 'Capture Face', !!capturedImage], ['3', 'Submit', result?.success]].map(([num, label, done], i) => (
          ...
          <div className="p-6 space-y-4">
            ...
            <div className={`rounded-xl border p-3 flex items-center gap-3 ${gpsStatus === 'ok' ? 'bg-emerald-50 border-emerald-200' : gpsStatus === 'error' ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-200'}`}>
              ...
              {gpsStatus !== 'loading' && gpsStatus !== 'ok' && (
                <button onClick={getGPS} className="text-xs text-indigo-600 font-medium hover:text-indigo-800 flex-shrink-0">Retry</button>
              )}
            </div>

            {gpsOk && (
              <div className="text-xs text-center text-gray-500 bg-gray-50 p-2 rounded-lg">
                📍 Must be within 100m of office to check in
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Live Webcam</p>
                {camStatus === 'ready' && (
              ...
              <div className="relative aspect-video bg-gray-900 rounded-xl overflow-hidden">
                {camStatus === 'captured' && capturedImage ? (
                  ...
                  <>
                    <video
                      ...
                      className={`w-full h-full object-cover transition-opacity ${camStatus === 'ready' ? 'opacity-100' : 'opacity-0'}`}
                    />
                    {camStatus !== 'ready' && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-white">
                        {camStatus === 'loading' && (
                          <>
                            <FaSpinner className="animate-spin text-2xl opacity-70" />
                        ...
                        {camStatus === 'error' && (
                          <>
                            ...
                            <p className="text-xs text-red-300 text-center px-4">
                              Camera access denied or unavailable.<br />Please allow camera permission and retry.
                            </p>
                            <button
                              onClick={() => { setCamStatus('idle'); }}
                              className="mt-1 px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs rounded-lg transition-colors">
                              Retry Camera
                            </button>
                          </>
                        )}
                ...
                {camStatus === 'ready' && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="w-36 h-44 border-2 border-white/70 rounded-full shadow-lg" />
                    <p className="absolute bottom-3 text-white/60 text-xs">Center your face in the oval</p>
                  </div>
                )}

                {/* Captured overlay tick */}
                {camStatus === 'captured' && (
                  <div className="absolute top-2 right-2 bg-emerald-500 text-white text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1">
                    <FaCheckCircle className="text-xs" /> Captured
                  </div>
                )}
              ...
              {camStatus === 'captured' ? (
                <button
                  onClick={retakePhoto}
                  className="w-full py-2 border border-gray-200 rounded-xl text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
                  <FaCamera className="text-xs" /> Retake Photo
                </button>
              ) : (
                <button
                  onClick={capturePhoto}
                  disabled={camStatus !== 'ready'}
                  className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition-colors">
                  <FaCamera className="text-xs" />
                  {camStatus === 'loading' ? 'Starting camera…' : camStatus === 'error' ? 'Camera unavailable' : 'Capture Photo'}
                </button>
              )}
            </div>
            ...
            {result && (
              <div className={`rounded-xl p-3 flex items-start gap-3 ${result.success ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
                ...
                <div className="flex-1">
                  ...
                  {result.confidence && (
                    <p className="text-xs text-emerald-600 mt-0.5">Confidence: {(result.confidence * 100).toFixed(1)}%</p>
                  )}
                  {result.stage && !result.success && (
                    <p className="text-xs text-red-500 mt-0.5 capitalize">
                      Failed at: {result.stage} check
                      {result.distance && ` (Distance: ${result.distance}m, Max: ${result.maxDistance || 100}m)`}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-lg border border-indigo-100 p-4">
              ...
              <div className="space-y-2">
                {[['Time', formatTime(record.approvedCheckIn)], ['Method', record.checkInRequest?.remarks?.includes('AI') ? 'AI Verified' : 'Standard']].map(([label, value]) => (
            ...
            <div className="bg-white rounded-lg border border-rose-100 p-4">
              ...
              <div className="space-y-2">
                {[['Time', formatTime(record.approvedCheckOut)], ['Method', record.checkOutRequest?.remarks?.includes('AI') ? 'AI Verified' : 'Standard']].map(([label, value]) => (
                  <div key={label} className="flex justify-between"><span className="text-xs text-gray-500">{label}</span><span className="text-xs font-medium text-gray-800">{value || '—'}</span></div>
                ))}
              </div>
            </div>
...
const EmployeeAttendanceContent = () => {
  ...
  const openDeleteModal  = (record) => {
    if (!isAdmin) { alert('Only administrators can delete attendance records'); return; }
    setRecordToDelete(record); setDeleteModalOpen(true);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-5 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-gray-400">
            ...
            <span className="text-sm">Attendance Portal</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          ...
          <KpiCard icon={FaCheckCircle} label="Completion Rate"  value={`${stats.totalDays > 0 ? ((stats.presentDays / stats.totalDays) * 100).toFixed(0) : 0}%`} sub="Present vs Total"               iconBg="bg-amber-500" />
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
            ...
            <div className="flex flex-col gap-3 min-w-[220px]">
              ...
              <p className="text-center text-xs text-gray-400 flex items-center justify-center gap-1">
                <FaMapMarkerAlt className="text-xs" /> Requires live webcam + GPS verification
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          {history.length === 0 && !loading ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">Check in using AI webcam to get started!</p>
            </div>
          ) : (
            <>
...
const EmployeeAttendance = () => (<ErrorBoundary><EmployeeAttendanceContent /></ErrorBoundary>);
