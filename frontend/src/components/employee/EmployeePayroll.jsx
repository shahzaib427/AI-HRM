...
const PayslipModal = ({
  ...
  const fetchPayslip = useCallback(async () => {
    ...
    try {
      console.log(
        '📄 Fetching payslip:',
        `/employee/payroll/${payroll._id}/payslip`
      );
      ...
      if (response.data) {
        ...
        setError('No payslip data received.');
      }
    } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    try {
      console.log(
        '⬇️ Downloading payslip:',
        `/employee/payroll/${payroll._id}/payslip/download`
      );
      ...
      link.setAttribute(
        'download',
        `payslip_${payroll.month}_${payroll.year}.html`
      );
...
const EmployeePayroll = () => {
  ...
  const fetchPayrolls = useCallback(async () => {
    try {
      ...
      console.log(
        '💰 Fetching employee payroll:',
        `/employee/payroll?${params.toString()}`
      );
      ...
      if (response.data?.success) {
        ...
        const totalNet = data.reduce(
          (sum, payroll) =>
            sum + calculateTotalSalary(payroll),
          0
        );
  ...
  const fetchYears = useCallback(async () => {
    try {
      console.log(
        '📅 Fetching payroll years'
      );

      const response =
        await axiosInstance.get(
          '/employee/payroll/years'
        );

      console.log(
        '✅ Payroll years response:',
        response.data
      );
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  View and download all your payslips
                </p>
              </div>
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-5 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              For payroll queries,
              discrepancies, or tax
              information, contact our HR
              team at{' '}
