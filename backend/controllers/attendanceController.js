...
// ── Face-match confidence floor ──────────────────────────────────
// Lowered from 0.70 → 0.65 so genuine employees checking in/out under
// dim lighting (which drags ArcFace confidence down into the 65-75%
// band even for a correct match) aren't wrongly rejected. Anything
// below 0.65 is still treated as a failed/uncertain match.
const MIN_CONFIDENCE   = parseFloat(process.env.FACE_MIN_CONFIDENCE || '0.65');
...
async function _processAIAttendance(req, res, type) {
  try {
    ...
    // Flag borderline accepts (65-75%) for visibility in remarks/audit,
    // without rejecting them — dim lighting commonly lands here.
    const isLowConfidence = aiResult.confidence < 0.75;
    ...
    const aiRemarks    = `AI verified — confidence: ${(aiResult.confidence * 100).toFixed(1)}%${isLowConfidence ? ' (borderline — possible dim lighting)' : ''}, GPS: ${gps.distance} m from office`;
    ...
    if (type === 'checkin') {
      ...
      return res.json({
        ...
        data:    attendance,
        ai:      { confidence: aiResult.confidence, gpsDistance: gps.distance, lowConfidence: isLowConfidence }
      });
    }
