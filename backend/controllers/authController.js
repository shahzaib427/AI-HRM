...
// =====================================================================
// ✅ EMAIL DELIVERY — SWITCHED FROM NODEMAILER/GMAIL SMTP TO RESEND API
// =====================================================================
// Why: Gmail SMTP from cloud hosts (Render, Vercel, Railway, etc.) is
// unreliable — connections routinely hang and time out because Google
// throttles/blocks SMTP traffic from data-center IP ranges. That's what
// was causing "❌ Welcome email error: Connection timeout" in the logs.
//
// Resend sends over plain HTTPS (port 443, always open, no SMTP handshake),
// so there's no port-blocking or timeout risk. It also has a generous free
// tier (3,000 emails/month).
//
// SETUP REQUIRED ON RENDER:
//   1. Sign up at https://resend.com (free tier is fine to start)
//   2. Verify a sending domain (Resend dashboard → Domains) OR use their
//      shared "onboarding@resend.dev" sender for testing only.
//   3. Create an API key (Resend dashboard → API Keys)
//   4. On Render → your service → Environment, set:
//        RESEND_API_KEY = re_xxxxxxxxxxxxxxxxxxxxxxxxxxxx
//        FROM_EMAIL     = HR System <hr@yourdomain.com>   (must match a
//                          verified domain in Resend, or use
//                          "onboarding@resend.dev" while testing)
//   5. You can now DELETE the old SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS
//      env vars — they are no longer used anywhere in this file.
// =====================================================================
...
async function sendViaResend({ to, subject, html }) {
  if (!process.env.RESEND_API_KEY) {
    console.error('❌ RESEND_API_KEY is not set — cannot send email. Set it in Render → Environment.');
    return false;
  }

  const fromAddress = process.env.FROM_EMAIL || 'HR System <onboarding@resend.dev>';
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`❌ Resend API error (status ${response.status}) sending to ${to}:`, data);
      return false;
    }
    ...
    // Network-level failure (should be rare with HTTPS, but handled defensively)
    console.error(`❌ Resend request failed for ${to}:`, error.message);
    return false;
  }
}
...
// ✅ This is the ONLY email-sending logic for new accounts. It's split into
// a plain internal function (sendWelcomeEmailInternal) that other controllers
// can call directly — no fake req/res "mock" objects — plus a thin route
...
// route calls, but userController calls sendWelcomeEmailInternal directly
